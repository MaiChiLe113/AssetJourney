...
export default function AssetJourneyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const monthlyContribution = (goal.target / (goal.daysLeft / 30) / 1e6).toFixed(0);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.gamifyBar}>
        <div className={styles.gamifyItem}>
          ...
          <span className={styles.gamifyVal}>{goal.streakDays} ngày</span>
        ...
        <div className={styles.gamifyItem}>
          ...
          <span className={styles.gamifyVal}>{goal.uPointsEarned.toLocaleString('vi-VN')}</span>
      ...
      <div className={styles.goalSection}>
        <Link href={`/asset-journey/goal/${goal.id}`} style={{ textDecoration: 'none', display: 'block' }}>
          <div className={styles.goalCard} style={{ backgroundImage: `url('${goal.bgImage}')` }}>
            ...
            <div className={styles.goalAmts}>
              <div>
                ...
                <div className={styles.goalAmtVal}>{(goal.saved / 1000000).toLocaleString('vi-VN')}M</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div className={styles.goalAmtLabel}>Mục tiêu</div>
                <div className={styles.goalAmtValTarget}>{(goal.target / 1000000).toLocaleString('vi-VN')}M</div>
              </div>
            </div>

            <div className={styles.progTrackWrapper}>
              <div className={styles.progFillGlow} style={{ width: `${goal.progress}%` }} />
            </div>
            
              <div className={styles.goalMeta}>
                <span className={styles.pctText}>{goal.progress}% Đã hoàn thành</span>
                <span className={styles.metaText}>~{Math.ceil(goal.daysLeft / 30)} tháng còn lại</span>
              </div>
            </div>
          </div>
        </Link>
        ...
        <div className={styles.goalStats}>
          <div className={styles.goalStat}>
            <div className={styles.goalStatVal}>₫{monthlyContribution}M</div>
            <div className={styles.goalStatLbl}>Góp/tháng</div>
          </div>
          <div className={styles.goalStatDiv} />
          <div className={styles.goalStat}>
            <div className={styles.goalStatVal}>{goal.rate}%</div>
