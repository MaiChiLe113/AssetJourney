  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.goalSection}>
        <div className={styles.goalCard} style={{ backgroundImage: "url('https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=800&q=80')" }}>
          <div className={styles.goalOverlay}>
            <div className={styles.goalTop}>
              <div className={styles.goalLeft}>
                <div className={styles.goalEyebrow}>Mục tiêu hiện tại</div>
                <div className={styles.goalTitle} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <Image src="/images/TechZAssetJourney.png" alt="house" width={24} height={24} style={{ background: "#fff", borderRadius: "50%", padding: "2px" }} />
                  {goal.label}
                </div>
              </div>
              <Link href="/asset-journey/setup" className={styles.goalEdit}>Sửa</Link>
            </div>

            <div className={styles.goalAmts}>
              <div>
                <div className={styles.goalAmtLabel}>Đã tích lũy</div>
                <div className={styles.goalAmtVal}>₫{fmtM(goal.saved)}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div className={styles.goalAmtLabel}>Mục tiêu</div>
                <div className={styles.goalAmtValTarget}>₫{(goal.target/1e9).toFixed(1)}B</div>
              </div>
            </div>

            <div className={styles.progTrackWrapper}>
              <div className={styles.progFillGlow} style={{ width: `${pct}%` }} />
            </div>
            
            <div className={styles.goalMeta}>
              <span className={styles.pctText}>{pct}% hoàn thành</span>
              <span className={styles.metaText}>~16 tháng còn lại</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: 12, padding: "10px 14px", background: "#fff", borderRadius: "var(--r-lg)", display: "flex", gap: "10px", alignItems: "flex-start", border: "1px solid rgba(237, 28, 36, 0.1)" }}>
          <Image src="/mascot/excited.png" alt="TekTek" width={28} height={28} style={{ flexShrink: 0 }} />
          <div style={{ fontSize: 12, color: "var(--g700)", lineHeight: 1.4 }}>
            <strong style={{ color: "var(--tcb-red)", display: "block", marginBottom: 2 }}>TekTek:</strong>
            Tuyệt vời! Bạn đang duy trì chuỗi 8 tháng tiết kiệm liên tiếp. Giữ vững phong độ này để tiến gần hơn đến ngôi nhà mơ ước nhé! 🚀
          </div>
        </div>
      </div>
